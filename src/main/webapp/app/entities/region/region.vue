<template>
  <div>
    <h2 id="page-heading" data-cy="RegionHeading">
      <span v-text="$t('jhipsterSampleApplicationApp.region.home.title')" id="region-heading">Regions</span>
      <div class="d-flex justify-content-end">
        <button class="btn btn-info mr-2" v-on:click="handleSyncList" :disabled="isFetching">
          <font-awesome-icon icon="sync" :spin="isFetching"></font-awesome-icon>
          <span v-text="$t('jhipsterSampleApplicationApp.region.home.refreshListLabel')">Refresh List</span>
        </button>
        <router-link :to="{ name: 'RegionCreate' }" custom v-slot="{ navigate }">
          ...
          >
            ...
            <span v-text="$t('jhipsterSampleApplicationApp.region.home.createLabel')"> Create a new Region </span>
          </button>
        </router-link>
      </div>
    </h2>
    ...
    <div class="alert alert-warning" v-if="!isFetching && regions && regions.length === 0">
      <span v-text="$t('jhipsterSampleApplicationApp.region.home.notFound')">No regions found</span>
    </div>
        ...
        <thead>
          <tr>
            ...
            <th scope="row" v-on:click="changeOrder('regionName')">
              <span v-text="$t('jhipsterSampleApplicationApp.region.regionName')">Region Name</span>
    ...
    <b-modal ref="removeEntity" id="removeEntity">
      ...
      <div class="modal-body">
        <p id="jhi-delete-region-heading" v-text="$t('jhipsterSampleApplicationApp.region.delete.question', { id: removeId })">
