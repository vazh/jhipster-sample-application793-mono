<template>
  <div>
    <h2 id="page-heading" data-cy="DepartmentHeading">
      <span v-text="$t('jhipsterSampleApplicationApp.department.home.title')" id="department-heading">Departments</span>
      <div class="d-flex justify-content-end">
        <button class="btn btn-info mr-2" v-on:click="handleSyncList" :disabled="isFetching">
          <font-awesome-icon icon="sync" :spin="isFetching"></font-awesome-icon>
          <span v-text="$t('jhipsterSampleApplicationApp.department.home.refreshListLabel')">Refresh List</span>
        </button>
        <router-link :to="{ name: 'DepartmentCreate' }" custom v-slot="{ navigate }">
          ...
          >
            ...
            <span v-text="$t('jhipsterSampleApplicationApp.department.home.createLabel')"> Create a new Department </span>
          </button>
        </router-link>
      </div>
    </h2>
    ...
    <div class="alert alert-warning" v-if="!isFetching && departments && departments.length === 0">
      <span v-text="$t('jhipsterSampleApplicationApp.department.home.notFound')">No departments found</span>
    </div>
        ...
        <thead>
          <tr>
            ...
            <th scope="row" v-on:click="changeOrder('departmentName')">
              <span v-text="$t('jhipsterSampleApplicationApp.department.departmentName')">Department Name</span>
            ...
            <th scope="row" v-on:click="changeOrder('location.id')">
              <span v-text="$t('jhipsterSampleApplicationApp.department.location')">Location</span>
    ...
    <b-modal ref="removeEntity" id="removeEntity">
      ...
      <div class="modal-body">
        <p id="jhi-delete-department-heading" v-text="$t('jhipsterSampleApplicationApp.department.delete.question', { id: removeId })">
